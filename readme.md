...
本项目主要是根据 Jean Meeus 的天文算法来计算天体位置，引用了实现该算法的库[soniakeys/meeus](https://github.com/soniakeys/meeus)。  
"VSOP87"数据文件作为天文算法中采用的VSOP87行星理论的必要文件，需要在此项目中作为环境变量导入使用。  
数据文件在本项目的 `VSOP87`目录中，可以下载到任何目录，添加环境变量`VSOP87`即可。
